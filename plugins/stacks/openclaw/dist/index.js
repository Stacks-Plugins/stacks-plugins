...
import { stacksConfigSchema } from './config.js';
...
export default defineToolPlugin({
    ...
    tools: (tool) => [
        tool({
            ...
            description: 'Show the configured Stacks wallet address, network, and signing status. ' +
                'Use when the user asks about "my wallet", signing readiness, or which network is active.',
            parameters: Type.Object({}),
            execute: (_params, config) => walletContextText(config),
        }),
        ...stacksToolSpecs.map((spec) => tool({
            name: spec.name,
            label: spec.label,
            ...
            async execute(params, config, context) {
                ...
                const { text, isError } = await executeTool(spec, params, { pluginConfig: config });
                if (isError) {
