 ...
 */
export async function bridgeQuote(params: BridgeQuoteParams): Promise<BridgeQuoteResult> {
  const url =
    `${ALLBRIDGE_API}/swap?amount=${params.amount}` +
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch bridge quote ${params.token} ${params.fromChain}->${params.toChain}: ` +
 ...
 */
export async function bridgeInitiate(_params: BridgeInitiateParams): Promise<BroadcastResult> {
  throw new Error(
    'bridgeInitiate is not auto-executed. Fetch a quote with `bridgeQuote`, then ' +
