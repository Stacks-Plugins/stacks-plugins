...
import {
  BroadcastResult,
  SbtcBuildPegInParams,
  SbtcBuildPegInResult,
  SbtcGetBalanceParams,
  SbtcGetBalanceResult,
  SbtcGetPegStatusParams,
  SbtcGetPegStatusResult,
  SbtcInitiatePegInParams,
  SbtcInitiatePegInResult,
  SbtcInitiatePegOutParams,
  SendSbtcParams,
} from '../types';
import { parseSbtcAmount } from '../utils/amounts';
import { btcAddressToRecipient } from '../utils/btcAddress';
import {
  bitcoinPrivateKeyBytes,
  bitcoinReclaimPublicKeyHex,
  getBitcoinAddress,
  getBitcoinPrivateKey,
} from '../utils/btcKeys';
import { readOnlyCall } from './contracts';

function sbtcClient(network: ReturnType<typeof resolveNetwork>) {
  return network === 'testnet' ? new SbtcApiClientTestnet() : new SbtcApiClientMainnet();
}

function btcNetwork(network: ReturnType<typeof resolveNetwork>) {
  return network === 'testnet' ? TESTNET : MAINNET;
}

function stacksAddressFromKey(
  senderKey: string,
...
export async function sbtcGetBalance(params: SbtcGetBalanceParams): Promise<SbtcGetBalanceResult> {
  ...
  const raw = balance.value as { type?: string; value?: string | number | { type?: string; value?: string | number } };
  let sbtc = '0';
  if (raw?.type === 'uint' && raw.value != null && typeof raw.value !== 'object') {
    ...
    sbtc = String((raw.value as { value?: string | number }).value ?? '0');
...
export async function sendSbtc(params: SendSbtcParams): Promise<BroadcastResult> {
  const network = resolveNetwork(params.network);
  const contracts = getNetworkContracts(network);
  ...
  if (sender === params.recipient) {
    throw new Error(
      'Recipient cannot equal sender: sBTC self-transfers have zero net token change and fail strict post-conditions (SentEq 0). Use a different recipient address.'
    );
  }

  const callOptions: Parameters<typeof makeContractCall>[0] = {
    contractAddress: contracts.sbtcToken.address,
    contractName: contracts.sbtcToken.name,
    ...
    postConditions: [
      Pc.principal(sender)
        .willSendEq(amount)
        .ft(contractId(contracts.sbtcToken) as `${string}.${string}`, 'sbtc-token'),
    ],
  };
  if (params.fee != null) callOptions.fee = BigInt(params.fee);
  if (params.nonce != null) callOptions.nonce = BigInt(params.nonce);
...
export async function sbtcBuildPegIn(
  params: SbtcBuildPegInParams
): Promise<SbtcBuildPegInResult> {
  const network = resolveNetwork(params.network);
  const contracts = getNetworkContracts(network);
  const client = sbtcClient(network);
  const signersPublicKey = await client.fetchSignersPublicKey(contracts.sbtcRegistry.address);
  const reclaimPublicKey =
    params.reclaimPublicKey ?? bitcoinReclaimPublicKeyHex(params.bitcoinPrivateKey);

  const deposit = buildSbtcDepositAddress({
    network: btcNetwork(network),
    stacksAddress: params.stacksAddress,
    signersPublicKey,
    maxSignerFee: params.maxSignerFee ?? 80_000,
...
export async function sbtcInitiatePegIn(
  params: SbtcInitiatePegInParams
): Promise<SbtcInitiatePegInResult> {
  const network = resolveNetwork(params.network);
  const contracts = getNetworkContracts(network);
  const client = sbtcClient(network);
  ...
  const stacksAddress =
    params.stacksAddress ??
    (params.senderKey ? stacksAddressFromKey(params.senderKey, network) : undefined);

  if (!stacksAddress) {
    throw new Error(
      'stacksAddress or senderKey is required to receive minted sBTC on Stacks.'
    );
  }

  const amountSats = Number(parseSbtcAmount(params.amount));
  if (amountSats <= 0) throw new Error('Peg-in amount must be greater than zero.');
...
export async function sbtcInitiatePegOut(
  params: SbtcInitiatePegOutParams
): Promise<BroadcastResult> {
  const network = resolveNetwork(params.network);
  const contracts = getNetworkContracts(network);
  const amount = parseSbtcAmount(params.amount);
  const maxFee = parseSbtcAmount(params.maxFee ?? params.maxSignerFee ?? 3_000);
  const sender = stacksAddressFromKey(params.senderKey, network);
...
export async function sbtcGetPegStatus(
  params: SbtcGetPegStatusParams
): Promise<SbtcGetPegStatusResult> {
  const network = resolveNetwork(params.network);
  const client = sbtcClient(network);
  ...
  if (params.stacksAddress) {
    const baseUrl = (client as any).config?.sbtcApiUrl ?? 'https://sbtc-emily.com';
    const res = await fetchFn(
      `${baseUrl}/withdrawal/sender/${encodeURIComponent(params.stacksAddress)}`
    );
    if (!res.ok) {
      throw new Error(
        `Failed to fetch withdrawal status for ${params.stacksAddress}: ${res.status} ${res.statusText}`
      );
    }
