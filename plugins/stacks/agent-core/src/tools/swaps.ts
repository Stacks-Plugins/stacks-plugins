...
export async function swapQuote(params: SwapQuoteParams): Promise<SwapQuoteResult> {
  ...
  const url =
    `${ALEX_API}/v1/price/${encodeURIComponent(params.tokenFrom)}/` +
    `${encodeURIComponent(params.tokenTo)}?amount=${params.amount}`;

  const res = await fetchFn(url);
  if (!res.ok) {
    throw new Error(
      `Failed to fetch swap quote ${params.tokenFrom}->${params.tokenTo}: ` +
  ...
  const route: string[] = json.route ?? [params.tokenFrom, params.tokenTo];
...
export async function swapExecute(
  params: SwapExecuteParams & { functionArgsHex?: string[]; functionName?: string }
): Promise<BroadcastResult> {
  if (!params.functionArgsHex || params.functionArgsHex.length === 0) {
    throw new Error(
      'swapExecute requires `functionArgsHex` encoding the ALEX route/amounts. ' +
