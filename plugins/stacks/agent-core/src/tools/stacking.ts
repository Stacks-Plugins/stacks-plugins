...
import {
  ...
  StackingStatusParams,
  StackParams,
} from '../types';

function clientFor(address: string, network: ReturnType<typeof resolveNetwork>) {
  return new StackingClient({ address, network, client: stacksClient(network) });
}

function toBroadcastResult(result: any): BroadcastResult {
...
export async function getStackingStatus(params: StackingStatusParams) {
...
export async function canStack(params: CanStackParams) {
  const network = resolveNetwork(params.network);
  const client = clientFor(params.address, network);
...
export async function stack(params: StackParams): Promise<BroadcastResult> {
  const network = resolveNetwork(params.network);
  ...
  const burnBlockHeight =
    params.burnBlockHeight ?? (await client.getPoxInfo()).next_cycle.prepare_phase_start_block_height;
...
export async function delegateStx(params: DelegateStxParams): Promise<BroadcastResult> {
  const network = resolveNetwork(params.network);
  const client = new StackingClient({
...
export async function revokeDelegate(params: RevokeDelegateParams): Promise<BroadcastResult> {
  const network = resolveNetwork(params.network);
  const client = new StackingClient({
    address: '',
    network,
