...
function decodePrivateKey(raw: string): Uint8Array {
  ...
  throw new Error(
    'BITCOIN_PRIVATE_KEY must be a 32-byte hex string or WIF-encoded private key.'
  );
}

export function getBitcoinPrivateKey(): string {
  const key = process.env.BITCOIN_PRIVATE_KEY?.trim();
  if (!key) {
    throw new Error(
      'BITCOIN_PRIVATE_KEY is required for Bitcoin peg-in. Set it in the environment.'
    );
  }
  return key;
}
...
export function getBitcoinAddress(): string {
  ...
  if (!address) {
    throw new Error(
      'BITCOIN_ADDRESS is required for Bitcoin peg-in. Set it in the environment.'
    );
  }
  return address;
}
...
export function bitcoinReclaimPublicKeyHex(privateKey?: string): string {
  const key = decodePrivateKey(privateKey ?? getBitcoinPrivateKey());
  const pub = pubSchnorr(key);
  return bytesToHex(pub);
}

export function bitcoinPrivateKeyBytes(privateKey?: string): Uint8Array {
