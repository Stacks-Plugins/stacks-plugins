...
| --- | --- | --- | --- |
| **ElizaOS** | Wraps handlers in `Action` objects via `makeAction()`; `STACKS_WALLET` provider | 19 | `senderKey` or `STACKS_SENDER_KEY` |
| **OpenClaw** | Registers tools with TypeBox schemas via `defineToolPlugin()` | 20 (+ `stacks_wallet_info`) | Marked `optional: true` in manifest |
| **Hermes** | Python handlers + Node subprocess bridge | 19 | `STACKS_SENDER_KEY` injected by bridge |
...
Every framework exposes the **same 19 core tool names** (e.g. `stacks_get_balance`, `stacks_send_tokens`). OpenClaw adds `stacks_wallet_info`. Parameters and return shapes match agent-core types so agents can be ported between frameworks without retraining on different schemas.
